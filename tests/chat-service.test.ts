...
test("dashboard session can start proposals through chat routes but not mutate runs directly", async () => {
  ...
  try {
    ...
    // Proposal /start is now session-allowed (ordinary actions).
...
test("approval-preview merge stage includes integration branch and task commits", async () => {
  const h = await startService();
  try {
    seedRunWithTask(h.store);
    const response = await fetch(
...
test("browser-confirm approve path accepts correct prefix and approves the run", async () => {
  ...
  try {
    ...
    const approveRes = await fetch(`${h.base}/api/v1/runs/proposal-run/approve`, {
      ...
      body: JSON.stringify({ stage: "plan", bindingHash, runVersion, confirm: bindingHash.slice(0, 8) }),
    });
...
test("browser-confirm approve path rejects stale runVersion with 409", async () => {
  ...
  try {
    ...
    const approveRes = await fetch(`${h.base}/api/v1/runs/proposal-run/approve`, {
      ...
      body: JSON.stringify({ stage: "plan", bindingHash, runVersion: 9999, confirm: bindingHash.slice(0, 8) }),
    });
...
test("browser-confirm approve path is accessible from a dashboard session", async () => {
  ...
  try {
    ...
    const approveRes = await fetch(`${h.base}/api/v1/runs/proposal-run/approve`, {
      method: "POST",
      headers: { cookie, "content-type": "application/json", "idempotency-key": "test-approve-session" },
      body: JSON.stringify({ stage: "plan", bindingHash, runVersion, confirm: bindingHash.slice(0, 8) }),
    });
    assert.equal(approveRes.status, 200);
    assert.equal(h.store.isApproved("proposal-run", "plan"), true);
...
test("dashboard session cannot use the action-ticket approval path", async () => {
  ...
  try {
    ...
    const approveRes = await fetch(`${h.base}/api/v1/runs/proposal-run/approve`, {
      method: "POST",
      headers: { cookie, "content-type": "application/json", "idempotency-key": "test-approve-session-ticket" },
      body: JSON.stringify({ stage: "plan", actionTicket: "some-fake-ticket", expectedVersion: 1 }),
    });
